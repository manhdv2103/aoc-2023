...
fn parse(input: String) -> Vec<(i32, Vec<Vec<(i32, String)>>)> {
  input
    .lines()
    .map(|game| {
      let (_, [game_id, game_info]) =
        Regex::new(r"Game (\d+): (.+)").unwrap().captures(game).unwrap().extract();
        ...
        game_info
          ...
          .map(|cube_set| {
            cube_set
              .split(", ")
              .map(|cube_type| {
                let (cube_num, cube_color) = cube_type.splitn(2, ' ').collect_tuple().unwrap();
  ...
  parse(input)
    ...
    .map(|(game_id, game_info)| {
      let can_be_played = game_info.iter().all(|bag| {
        bag
          .iter()
          .all(|(cube_num, cube_color)| cube_num <= real_bag.get(cube_color.as_str()).unwrap())
      });
...
fn solve_p2(input: String) -> String {
  return parse(input)
    .iter()
    .map(|(_, game_info)| {
      let max_cubes = game_info.iter().fold(HashMap::new(), |mut max_cubes, cube_set| {
        {
